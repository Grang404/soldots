				...
				lua_ls = {
					...
					settings = {
						Lua = {
							...
							diagnostics = {
								globals = { "vim" },
								-- Optional: Disable specific diagnostics
								disable = {
									"undefined-global",
									-- "missing-fields", -- Uncomment to disable missing fields warnings
								},
							},
							workspace = {
	...
	{ -- Harpoon
		...
		config = function()
			vim.api.nvim_set_keymap(
				...
				{ noremap = true, silent = true, desc = "[A]dd file to Harpoon" }
			)
			vim.api.nvim_set_keymap(
				"n",
				"<leader>hq",
				':lua require("harpoon.ui").toggle_quick_menu()<CR>',
				{ noremap = true, silent = true, desc = "[Q]ick Menu" }
			)

			-- Create key mappings for navigating to files 0-9
			for i = 0, 9 do
				vim.api.nvim_set_keymap(
					"n",
					"<leader>h" .. i,
